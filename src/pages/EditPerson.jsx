...
import CommentSection from '../components/CommentSection'; // Импорт компонента для комментариев

const EditPerson = () => {
  const { id } = useParams(); // Получаем ID человека
  ...
  return (
    <div>
      ...
      <CommentSection personId={id} /> {/* Передаем ID человека для комментариев */}
    </div>
  );
};
